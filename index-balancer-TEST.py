...
# This test code has an additional scaler that uses an ensemble of  dual moving average
# crossovers to additionally scale the target asset amount held in order
# to allow the code to follow trends for each asset. It uses the ensemble to
# scale in across a few averages to generalize/regularize and reduce any
# overfitting in theory.
# This code is currently (8/2021) in A/B testing for performance along with
# the original version working in parralel in 2 portfolios.

#author			:Erick Clasen
#date			:20210822
#version		:0.1
#usage			:python index-balancer-TEST.py key b64secret passphrase
#notes			:
...
for asset in asset_dict:
        ...
        if action == 'BUY':
            ...
            if underlying_avail < trade_amount:
                    print("Not enough "+str(underlying)+" available to buy, break! ",underlying_avail)
                    continue
            ...
            if asset_dict[asset] < trade_amount:
                    print("Not enough "+asset+"-"+str(underlying)+" available to sell, break! ",underlying_avail)
                    continue
            size,buy_price,sell_price = cbs.calculate_trade_size_and_price(asset,current_prices[asset+" Price"],trade_amount,asset_dict[asset])
